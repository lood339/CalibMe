...
angle_noise_level = [0.0:0.2:1.0];
angle_noise_level = angle_noise_level * pi / 180.0;
n_angle = length(angle_noise_level);
angular_mean_std = zeros(n_angle, 2);
fl_mean_std = zeros(n_angle, 2);
for i = [1:n_angle]
    N = 200;
    ...
    for j = [1:N]
        ...
        % add noise to base rotation
        angle_noise = normrnd(0, angle_noise_level(i));        
        rotation_noise = axang2rotm([base_rot_axis(1), base_rot_axis(2), base_rot_axis(3), angle_noise]);
        rotation = rotation_noise * base_rot;        
        noisy_rod = rotationMatrixToVector(rotation');
 
        noisy_camera = ptz_to_camera(ptz, pp, cc, noisy_rod);
